...
def most( key )
  ...
  twitter_data.each do |data|
    ...
    if data[username][key] > last_num
      name = username
      last_num = data[name][key]
    end
  end
...
twitter_data.each do |data|
  username = data.keys.first
  description = data[username]["description"] || "N/A"

  puts "#{ username }: #{ description }"
end
...
twitter_data.each do |data|
  username = data.keys.first
  followers = data[username]["number of followers"]

  puts "#{ username }: #{ followers }"
end
...
twitter_data.each do |data|
  username = data.keys.first
  latest = data[username]["latest tweet"].length

  puts "#{ username }'s lastest tweet was #{ latest } characters long"
end
...
twitter_data.each do |data|
  ...
  data[username]["last twenty tweets"].each do |tweet|
    tweet_length += tweet.length
  end

  puts "#{ username } used #{ tweet_length } characters in their last 20 tweets"
end

puts

most_follows = most( 'number of followers' )
puts "#{ most_follows } has the most followers"

most_friends = most( "number of friends" )
puts "#{ most_friends } has the most friends"

most_tweets = most( "number of tweets" )
puts "#{ most_tweets } has the most tweets"

puts
puts "Users with a description: "
twitter_data.select { |data| data[data.keys.first]["description"] }.each do |user|
  puts "> #{ user.keys.first }"
end

puts
puts "Users with a location listed: "
twitter_data.select { |data| data[data.keys.first]["location"] }.each do |user|
  puts "> #{ user.keys.first }"
end
