...
puts "There are #{ people.count } people in the hash"
# * What are the names of all the people?
puts "All the people: "
people.keys.each { |person| puts "> #{ person }" }
# * What is the phone number of Alia O'Conner PhD?
puts "Alia's phone number is: #{ people["Alia O'Conner PhD"]["phone"] }"
# * How many children does Brian Heller have?
puts "Brain has #{ people["Brian Heller"]["children"].count } child"
# * What company does Dr. Adela DuBuque work for?
puts "Dr. Adela works for #{ people["Dr. Adela DuBuque"]["company"] }"
# * What are the names of the people who have children?
puts "Names of the people that have children: "
people.select{ |key,value| value.has_key? "children" }.each do |key,value|
  puts "> #{ key }"
end
# * What are the names of the people who do not have children?
puts "Names of the people that don't have children: "
people.select{ |key,value| !value.has_key? "children" }.each do |key,value|
  puts "> #{ key }"
end
# * What is Brian Heller's child's name?
puts "Brain's childs name is #{ people["Brian Heller"]["children"][0] }"
# * What is Maryse Johns' first child's name?
puts "Maryse's first childrens name is #{ people["Maryse Johns"]["children"][0] }"
...
puts "The first set of people has: #{ sets_of_people[0].count } people"
# * How many people are in the second set of people?
puts "The second set of people has: #{ sets_of_people[1].count } people"
# * What is Ruby Hessel's phone number?
puts "Ruby Hessel's phone number: #{ sets_of_people[0]["Ruby Hessel"]["phone"] }"
# * What are the names of Dr. Sigrid Nader's children?
puts "Dr. Sigrid Nader's children names are: "
...
sets_of_people[0]["Bernard Feil"]["children"].each { |child| puts "> #{ child }" }
# * What company does Casandra Kemmer work for?
puts "Casandra works for: #{ sets_of_people[0]["Casandra Kemmer"]["company"] }"
# * Who are all the people in the first set that have an `m` in their Company names?
puts "People in the first set that have an `m` in their company's name"
sets_of_people[0].each do |key,value|
  puts value["company"] if value["company"].include? "m"
end